...
export const updateUserInfo = CatchAsyncError(async(req:AuthenticatedRequest, res:Response, next:NextFunction) => {
    try {
        const {name,email} = req.body as MainUpdateUserInfo;
        const userId = req.user?._id;
...
export const updatePassword = CatchAsyncError(async(req:AuthenticatedRequest, res:Response, next:NextFunction) => {
    try {
        const {oldPassword, newPassword} = req.body as MainUpdatePassword;
        ...
        if(user?.password === undefined) {
            return next(new ErrorHandler("Invalid user", 400));   // if social authenticated user tries to change pass
        }
...
interface MainUpdateProfilePicture {
    avatar: string;
}
    ...
    try {
        ...
        if(avatar && user) {
            if(user?.avatar?.public_id) {
                ...
                await cloudinary.v2.uploader.destroy(user?.avatar?.public_id); // delete old image first

                const myCloud = await cloudinary.v2.uploader.upload(avatar, { // then upload new image
                    folder:"avatars",
                    width: 150,
                   });
